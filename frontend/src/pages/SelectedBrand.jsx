...
export default function SelectedBrand() {
  ...
  const generateLogo = useCallback(async () => {
    if (!selectedConcept || loading) return;
    try {
      setLoading(true);
      const res = await fetch("http://localhost:5000/api/generate-logo", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12 px-6 text-right" dir="rtl">
      <div className="max-w-md mx-auto bg-white p-8 rounded-3xl shadow-xl text-center">
        <h1 className="text-3xl font-bold mb-6">{selectedConcept?.brand_name_hebrew}</h1>
        <div className="aspect-square bg-slate-50 rounded-2xl mb-6 border-2 border-dashed flex items-center justify-center overflow-hidden">
          {loading ? <div className="animate-spin h-8 w-8 border-4 border-indigo-500 border-t-transparent rounded-full" /> : 
          logo && <img src={`data:image/png;base64,${logo}`} className="w-full h-full object-contain p-8" />}
        </div>
        <div className="flex flex-col gap-3">
          <button onClick={() => navigate("/landingBuilder", { state: getPayload() })} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold shadow-lg">המשך לדף נחיתה ✨</button>
          <button onClick={() => navigate("/posters", { state: getPayload() })} className="w-full py-3 bg-slate-900 text-white rounded-2xl font-bold">עבור לפוסטרים 🖼️</button>
          <div className="flex gap-2">
            <button onClick={() => {const a=document.createElement("a");a.href=`data:image/png;base64,${logo}`;a.download="logo.png";a.click();}} className="flex-1 py-3 border rounded-xl font-semibold">⬇️ הורדה</button>
            <button onClick={() => window.print()} className="flex-1 py-3 border rounded-xl font-semibold">🖨️ הדפסה</button>
          </div>
        </div>
      </div>
    </div>
